...
describe('Integration Tests', () => {
  ...
  it('should test an Signin endpoint', async () => {
    const response = await request(app).post('/auth/signin').send({username:testUser.username,password:testUser.password}); 
    expect(response.statusCode).to.equal(200);
    expect(response.body).to.deep.equal({message:StatusEnum.SUCCESS,username:testUser.username});
  });


  it('should test an delete endpoint', async () => {
    const response = await request(app).delete(`/auth/user/${testUser.username}`).send({username:testUser.username,password:testUser.password});
